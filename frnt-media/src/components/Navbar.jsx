...
export default function Navbar({ onTalk }) {
  ...
  // ✅ Body overflow — Navbar handles only its own menu (ContactModal handles its own separately)
  useEffect(() => {
    ...
    <>
      <nav style={{
        ...
        <div style={{fontFamily:'Bebas Neue',fontSize:'1.85rem',letterSpacing:'.08em',display:'flex',alignItems:'center',lineHeight:1,zIndex:201}}>
          ...
          <span style={{color:'var(--y)'}}>r</span>
          <span style={{color:'var(--p)'}}>n</span>
          <span style={{color:'var(--v)'}}>t</span>
        ...
        {/* Desktop CTA */}
        <button className="nav-cta" onClick={onTalk}
        ...
        <button
          ...
          <span style={{width:22,height:2,background:open?'var(--p)':'var(--white)',borderRadius:2,transition:'all .3s',transform:open?'rotate(45deg) translate(5px,5px)':'none',display:'block'}}/>
          <span style={{width:22,height:2,background:open?'var(--p)':'var(--white)',borderRadius:2,transition:'all .3s',opacity:open?0:1,display:'block'}}/>
          <span style={{width:22,height:2,background:open?'var(--p)':'var(--white)',borderRadius:2,transition:'all .3s',transform:open?'rotate(-45deg) translate(5px,-5px)':'none',display:'block'}}/>
        </button>
      </nav>
      ...
      <div style={{
        ...
        {links.map(l => (
          <a key={l}
            ...
            style={{fontFamily:'Bebas Neue',fontSize:'3rem',letterSpacing:'.1em',color:'rgba(255,255,255,.5)',transition:'color .2s',WebkitTapHighlightColor:'transparent'}}
            onTouchStart={e => e.currentTarget.style.color='var(--white)'}
            onTouchEnd={e => e.currentTarget.style.color='rgba(255,255,255,.5)'}
            onMouseEnter={e => e.target.style.color='var(--white)'}
            onMouseLeave={e => e.target.style.color='rgba(255,255,255,.5)'}>{l}</a>
        ))}
        <button
          onClick={() => { onTalk(); setOpen(false) }}
          style={{
            marginTop:8,
            background:'var(--p)',color:'var(--black)',
            padding:'14px 40px',border:'none',borderRadius:2,
